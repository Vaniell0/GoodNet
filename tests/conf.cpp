...
TEST(ConfigTest, LoadAllSections) {
    auto p = tmp_config(R"({
        ...
        "identity": {"dir":"/home/test/.gn","ssh_key_path":"/home/test/.ssh/id","use_machine_id":false,"skip_ssh_fallback":true},
        "ice": {"stun_servers":"stun.example.com:3478,stun2.example.com:3479","session_timeout":15,"keepalive_interval":30,"consent_max_failures":5}
    })");
    Config cfg(true);
    ASSERT_TRUE(cfg.load_from_file(p));
    ...
    EXPECT_TRUE(cfg.identity.skip_ssh_fallback);

    EXPECT_EQ(cfg.ice.stun_servers, "stun.example.com:3478,stun2.example.com:3479");
    EXPECT_EQ(cfg.ice.session_timeout, 15);
    EXPECT_EQ(cfg.ice.keepalive_interval, 30);
    EXPECT_EQ(cfg.ice.consent_max_failures, 5);
...
TEST(ConfigTest, IceDefaults) {
    Config cfg(true);
    EXPECT_EQ(cfg.ice.stun_servers,
              "stun.l.google.com:19302,stun1.l.google.com:19302,stun2.l.google.com:19302");
    EXPECT_EQ(cfg.ice.session_timeout, 10);
    EXPECT_EQ(cfg.ice.keepalive_interval, 20);
    EXPECT_EQ(cfg.ice.consent_max_failures, 3);
}

TEST(ConfigTest, LoadIceSection) {
    auto p = tmp_config(R"({
        "ice": {
            "stun_servers": "custom.stun:5000,another.stun:5001",
            "session_timeout": 30,
            "keepalive_interval": 60,
            "consent_max_failures": 10
        }
    })");
    Config cfg(true);
    ASSERT_TRUE(cfg.load_from_file(p));
    EXPECT_EQ(cfg.ice.stun_servers, "custom.stun:5000,another.stun:5001");
    EXPECT_EQ(cfg.ice.session_timeout, 30);
    EXPECT_EQ(cfg.ice.keepalive_interval, 60);
    EXPECT_EQ(cfg.ice.consent_max_failures, 10);
    fs::remove(p);
}

TEST(ConfigTest, IceRoundTrip) {
    Config cfg(true);
    cfg.ice.stun_servers       = "my.stun:1234";
    cfg.ice.session_timeout    = 42;
    cfg.ice.keepalive_interval = 99;
    cfg.ice.consent_max_failures = 7;

    auto json = cfg.save_to_string();
    Config cfg2(true);
    ASSERT_TRUE(cfg2.load_from_string(json));
    EXPECT_EQ(cfg2.ice.stun_servers, "my.stun:1234");
    EXPECT_EQ(cfg2.ice.session_timeout, 42);
    EXPECT_EQ(cfg2.ice.keepalive_interval, 99);
    EXPECT_EQ(cfg2.ice.consent_max_failures, 7);
}

TEST(ConfigTest, GetRaw_IceKeys) {
    Config cfg(true);
    // CSV string returned directly
    EXPECT_EQ(cfg.get_raw("ice.stun_servers"),
              "stun.l.google.com:19302,stun1.l.google.com:19302,stun2.l.google.com:19302");
    EXPECT_EQ(cfg.get_raw("ice.session_timeout"), "10");
    EXPECT_EQ(cfg.get_raw("ice.keepalive_interval"), "20");
    EXPECT_EQ(cfg.get_raw("ice.consent_max_failures"), "3");
    // Alias
    EXPECT_EQ(cfg.get_raw("ice.consent_failures"), "3");
    // Legacy single-server aliases
    EXPECT_EQ(cfg.get_raw("ice.stun_server"), "stun.l.google.com");
    EXPECT_EQ(cfg.get_raw("ice.stun_port"), "19302");
}

TEST(ConfigTest, GetRaw_IdentitySkipSshFallback) {
    Config cfg(true);
    EXPECT_EQ(cfg.get_raw("identity.skip_ssh_fallback"), "false");
    cfg.identity.skip_ssh_fallback = true;
    EXPECT_EQ(cfg.get_raw("identity.skip_ssh_fallback"), "true");
}
