...
TEST(PerConnQueueTest, TryPush_ExactLimit_Succeeds) {
    PerConnQueue q;
    // Push exactly MAX_BYTES (8 MB) in one frame
    std::vector<uint8_t> big_frame(PerConnQueue::MAX_BYTES, 0xDD);
    EXPECT_TRUE(q.try_push(std::move(big_frame)));
    EXPECT_EQ(q.pending_bytes.load(), PerConnQueue::MAX_BYTES);
    // One more byte should fail
    EXPECT_FALSE(q.try_push(std::vector<uint8_t>(1, 0xEE)));
}

TEST(PerConnQueueTest, DrainBatch_PartialDrain) {
    PerConnQueue q;
    for (int i = 0; i < 5; ++i) {
        q.try_push(std::vector<uint8_t>(64, static_cast<uint8_t>(i)));
    }
    EXPECT_EQ(q.pending_bytes.load(), 320u);

    auto batch = q.drain_batch(2);
    EXPECT_EQ(batch.size(), 2u);
    EXPECT_EQ(q.pending_bytes.load(), 192u);  // 3 * 64

    // Verify remaining frames are still there
    auto rest = q.drain_batch(64);
