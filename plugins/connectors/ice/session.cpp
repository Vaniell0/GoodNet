...
IceSession::IceSession(boost::asio::io_context& io,
    ...
    , keepalive_timer_(io)
{
    // Dual-stack: try IPv6 with v6_only(false) for IPv4+IPv6, fallback to IPv4
...
void IceSession::gather_host_candidates() {
    ...
    // Определяем наш IP через connect trick к первому STUN серверу
    boost::asio::ip::address local_addr;
    for (const auto& srv : config_.stun_servers) {
        boost::asio::ip::udp::resolver resolver(io_);
        auto results = resolver.resolve(srv.host, std::to_string(srv.port), ec);
        if (ec || results.empty()) continue;

        auto proto = results.begin()->endpoint().protocol();
        boost::asio::ip::udp::socket probe(io_, proto);
...
void IceSession::try_next_stun_server() {
    if (stun_try_idx_ >= config_.stun_servers.size()) {
        // Все STUN серверы не ответили — продолжаем без srflx
        on_srflx_result(std::nullopt);
        return;
    }

    const auto& srv = config_.stun_servers[stun_try_idx_];
    auto self = shared_from_this();
    stun_client_.resolve(srv.host, srv.port, socket_,
        [self](std::optional<StunAddress> addr) {
            if (addr) {
                self->on_srflx_result(std::move(addr));
            } else {
                // Этот сервер не ответил — пробуем следующий
                ++self->stun_try_idx_;
                self->try_next_stun_server();
            }
        });
}
...
void IceSession::send_keepalive() {
    ...
    keepalive_timer_.async_wait([self](const boost::system::error_code& ec) {
        if (ec) return;
        if (self->consent_missed_ >= self->config_.consent_max_failures) {
...
void IceSession::start_session_timeout() {
    timeout_timer_.expires_after(config_.session_timeout);
    auto self = shared_from_this();
