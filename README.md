...
Большинство сетевых решений в C++ решают одну из двух задач:
- дают максимальный контроль, но заставляют писать всё вручную (чистый Boost.Asio, sockpp);
- дают удобство и модульность, но скрывают транспорт и убирают ранний reject (POCO, Envoy, QuantumGate, ZeroMQ).

В результате инженеры либо тратят месяцы на низкоуровневый boilerplate, либо жертвуют безопасностью и гибкостью протоколов.

GoodNet создан именно для этой пустой ниши.

Он даёт **одновременно**:
- полный raw-доступ к сокетам на этапе `accept()` (можно отвергнуть соединение в первую миллисекунду);
- симметричную модель узлов без жёсткого деления клиент/сервер;
- динамические плагины (Handler + Connector) с обязательной SHA-256 верификацией и изоляцией;
- поддержку произвольных транспортов (TCP, UDP, Bluetooth, LoRa, proprietary radio, WebSocket — всё через один интерфейс);
- детерминированный shutdown и воспроизводимую сборку через Nix.

Никакой другой открытый проект на C++23 не предлагает эту комбинацию.  
Все существующие решения сознательно отказываются от одного или нескольких пунктов выше.

GoodNet — это «программная Cisco в коробке»: универсальный строительный блок, из которого за часы собирается и маршрутизатор, и IoT-шлюз, и защищённый прокси, и mesh-нода.
