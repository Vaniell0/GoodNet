...
См. также: [Noise_XX handshake](data/projects/GoodNet/docs/protocol/noise-handshake.md) · [Криптография](data/projects/GoodNet/docs/protocol/crypto.md) · [ConnectionManager](../architecture/connection-manager.md)
...
**Отличия от v2**: удалены `timestamp` (8 байт) и `sender_id` (16 байт). Timestamp не использовался на практике. Sender_id больше не нужен — [Noise_XX](data/projects/GoodNet/docs/protocol/noise-handshake.md) сессия привязана к peer identity через DH-операции, идентификация отправителя до дешифрации не требуется.

**packet_id** — монотонный per-connection counter. Двойное назначение: [AEAD nonce](data/projects/GoodNet/docs/protocol/crypto.md#nonce) (4 нулевых байта + 8 байт LE = 12-байтовый nonce) и дедупликация при [relay](data/projects/GoodNet/docs/architecture/connection-manager.md#gossip-relay).
...
Zstd включается автоматически для payload > 512 байт (настраивается в [compression config](data/projects/GoodNet/docs/config.md#compressionconfig)), если сжатый размер меньше оригинала. Иначе отправляется RAW.
...
|-----------|---------|---------|
| `MSG_TYPE_NOISE_INIT` | 1 | Noise msg1 (→e) + [HandshakePayload](data/projects/GoodNet/docs/protocol/noise-handshake.md#handshakepayload) |
| `MSG_TYPE_NOISE_RESP` | 2 | Noise msg2 (←e,ee,s,es) + encrypted payload |
| `MSG_TYPE_NOISE_FIN` | 3 | Noise msg3 (→s,se) + encrypted payload |
| `MSG_TYPE_HEARTBEAT` | 4 | [HeartbeatPayload](data/projects/GoodNet/docs/architecture/connection-manager.md#heartbeat) (16 bytes) |
| `MSG_TYPE_RELAY` | 10 | [RelayPayload](data/projects/GoodNet/docs/architecture/connection-manager.md#gossip-relay)(33) + inner_frame |
...
**См. также:** [Noise_XX handshake](data/projects/GoodNet/docs/protocol/noise-handshake.md) · [Криптография](data/projects/GoodNet/docs/protocol/crypto.md) · [ConnectionManager](../architecture/connection-manager.md) · [Connector: гайд](../guides/connector-guide.md)
