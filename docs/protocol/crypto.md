...
См. также: [Noise_XX handshake](../protocol/noise-handshake.md) · [Wire format](../protocol/wire-format.md) · [Конфигурация](../config.md)
...
Хранится в `~/.goodnet/user_key` (или указанный путь). Можно импортировать из OpenSSH Ed25519 (`identity.ssh_key_path` в [конфиге](../config.md#identitysection)). Переносится между устройствами — это ваш «аккаунт» в сети.
...
`NodeIdentity::load_or_generate()` (`core/cm/identity.cpp`) — при первом запуске генерирует ключи, при последующих — загружает.
...
X25519-версия device key используется как `s` (static keypair) в Noise [HandshakeState](../protocol/noise-handshake.md).
...
12-байтный nonce формируется из `packet_id` [заголовка](../protocol/wire-format.md#header_t-v3):
...
// Replay protection (NonceWindow — sliding bitmap)
if (!recv_window.accept(pkt_id)) return {};  // replay / out-of-window → drop
...
### Replay protection (NonceWindow)

Replay protection реализована через **NonceWindow** — sliding bitmap window (IPsec/DTLS-style), определённый в `core/types/nonce_window.hpp`.

**NonceWindow** допускает **out-of-order** доставку пакетов в пределах окна. Это **не** strict monotonic counter — пакеты могут приходить в произвольном порядке, если разница nonce не превышает размер окна.

Параметры:
- `WINDOW_SIZE = 256` — максимально допустимый разрыв между текущим nonce и самым высоким принятым
- `highest_nonce` — наибольший принятый nonce
- `bitmap[256]` — bitset, отслеживающий принятые nonce в пределах окна

Алгоритм `accept(nonce)`:
- `nonce == 0` → отклонить (зарезервировано)
- `nonce > highest_nonce` → принять, сдвинуть bitmap, обновить highest_nonce
- `highest_nonce - nonce >= WINDOW_SIZE` → отклонить (слишком старый, за пределами окна)
- `bitmap[diff] уже установлен` → отклонить (дубликат / replay)
- Иначе → принять, установить бит в bitmap

Потокобезопасность обеспечивается мьютексом (`std::mutex`). При rekey окно сбрасывается через `reset()`.
...
```
Время  Операция                           Состояние NonceWindow        Результат
─────  ──────────────────────────────────  ──────────────────────────── ──────────
  t0   Легитимный пакет (pkt_id=5)        highest=5, bitmap[0]=1       ✅ accepted
       ├─ accept(5): первый nonce
       └─ dispatch_packet()

  t1   Легитимный пакет (pkt_id=6)        highest=6, bitmap[0]=1,[1]=1 ✅ accepted
       ├─ accept(6): 6 > highest(5)
       └─ dispatch_packet()

  t2   Eve replay пакет (pkt_id=5)        highest=6                    ❌ REJECTED
       ├─ accept(5): diff=1, bitmap[1]=1
       └─ return {} (дубликат)            → packet dropped

  t3   Out-of-order пакет (pkt_id=8)      highest=8, bitmap сдвинут    ✅ accepted
       ├─ accept(8): 8 > highest(6), shift=2
       └─ dispatch_packet()

  t4   Запоздалый пакет (pkt_id=7)        highest=8, bitmap[1]=1       ✅ accepted
       ├─ accept(7): diff=1, bitmap[1]=0 → принят
       └─ dispatch_packet()               (out-of-order допустим)
```

**Код защиты (из NoiseSession::decrypt):**

```cpp
// NonceWindow::accept() — sliding bitmap (WINDOW_SIZE=256)
...
1. `accept()` вызывается **до** дешифрации → нет side channels
2. Mutex в NonceWindow → потокобезопасность
3. Bitmap отслеживает каждый принятый nonce → повторная отправка невозможна
4. Nonces старше WINDOW_SIZE (256) от highest → отклоняются безусловно

## NoiseSession

После завершения [Noise handshake](../protocol/noise-handshake.md) создаётся `NoiseSession`:
...
| Replay attack | NonceWindow (sliding bitmap, WINDOW_SIZE=256) + AEAD nonce из packet_id |
| Man-in-the-middle | [Noise handshake](../protocol/noise-handshake.md) + [cross-verification](../protocol/noise-handshake.md#cross-verification) (Ed25519 → X25519) |
...
- **Endpoint correlation**: IP адреса не маскируются (нет onion routing)
  ...
  - Даже с [relay](../architecture/connection-manager.md#gossip-relay), прямые TCP соединения раскрывают graph структуру
...
GoodNet в текущем виде (Alpha 0.1.0) подходит для защиты от пассивного прослушивания и оппортунистического MitM (ISP DPI, публичный WiFi). Для threat models с активным государственным противником требуется дополнительная защита (PKI, traffic padding, onion routing) — см. [roadmap](../roadmap.md).
