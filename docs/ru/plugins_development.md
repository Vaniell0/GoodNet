...
`PluginManager::load_all_plugins()` верифицирует SHA-256 каждого `.so` по манифесту перед вызовом `dlopen`. Плагин с отсутствующим или несовпадающим манифестом пропускается с записью об ошибке в лог.
