...
PluginManager::PluginManager(host_api_t* api, fs::path plugins_base_dir)
    : impl_(std::make_unique<Impl>(api, std::move(plugins_base_dir)))
{
    LOG_INFO("PluginManager: base={}",
             impl_->plugins_base_dir.empty() ? "(none)" : impl_->plugins_base_dir.string());
}
...
std::expected<void, std::string> PluginManager::load_plugin(const fs::path& path) {
    LOG_TRACE("load_plugin: {}", path.string());
    if (!fs::exists(path))
    ...
    if (h_sym) {
        LOG_TRACE("load_plugin: found handler_init in '{}'", path.filename().string());
        auto info    = std::make_unique<HandlerInfo>();
        info->lib    = std::move(lib);
        info->api  = *impl_->host_api;
        info->api.internal_logger  = impl_->host_api->internal_logger;
        ...
        if (impl_->handlers.contains(name))
        ...
        impl_->handlers[name] = std::move(info);
        return {};
    }
    ...
    if (c_sym) {
        LOG_TRACE("load_plugin: found connector_init in '{}'", path.filename().string());
        ...
        std::unique_lock lock(impl_->rw_mutex);
        if (impl_->connectors.contains(scheme))
        ...
        impl_->connectors[scheme] = std::move(info);
        return {};
    }
...
void PluginManager::load_all_plugins() {
    if (impl_->plugins_base_dir.empty()) return;

    // Рекурсивный обход всех файлов в папке plugins
    for (const auto& entry : fs::recursive_directory_iterator(impl_->plugins_base_dir)) {
...
void PluginManager::unload_all() {
    if (!impl_) return;
    std::unique_lock lock(impl_->rw_mutex);
    LOG_TRACE("unload_all: {} handlers, {} connectors",
              impl_->handlers.size(), impl_->connectors.size());
