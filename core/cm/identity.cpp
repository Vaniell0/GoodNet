...
bool NodeIdentity::try_load_ssh_key(const fs::path& path,
    ...
    LOG_TRACE("identity: SSH key type={}", kt);
    if (kt != "ssh-ed25519") {
...
NodeIdentity NodeIdentity::load_or_generate(const Config::Identity& cfg) {
    if (sodium_init() < 0) throw std::runtime_error("libsodium init failed");
    const fs::path dir(cfg.dir);
    LOG_TRACE("identity: loading from dir={}", dir.string());
    ...
    if (!cfg.ssh_key_path.empty()) {
        LOG_TRACE("identity: trying SSH key '{}'", cfg.ssh_key_path);
        loaded = try_load_ssh_key(fs::path(cfg.ssh_key_path), id.user_pubkey, id.user_seckey);
    }
    ...
    if (!loaded)
        load_or_gen_keypair(dir / "user_key", id.user_pubkey, id.user_seckey);

    if (cfg.use_machine_id) {
        const std::string mid = MachineId::get_or_create(dir);
        LOG_TRACE("identity: machine_id={}...", mid.substr(0, 12));
        if (!mid.empty()) {
            MachineId::derive_device_keypair(mid, id.user_pubkey,
                                              id.device_pubkey, id.device_seckey);
            LOG_INFO("Device key: hardware-bound");
        } else {
            load_or_gen_keypair(dir / "device_key", id.device_pubkey, id.device_seckey);
        }
    } else {
        load_or_gen_keypair(dir / "device_key", id.device_pubkey, id.device_seckey);
    }
